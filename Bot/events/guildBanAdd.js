module.exports = function (client, tempDB) {
  ...
  client.on("guildMemberRemove", async (member) => {
    try {
      ...
      const audit = (
        await member.guild.fetchAuditLogs({ limit: 1 })
      ).entries.first();
      ...
      if (
        db1.get({ key: `${member.guild.id}_WHITELISTS` }).includes(executor.id)
      )
        return;

      if (audit.action == "MEMBER_BAN_ADD") {
        let banAdd = tempDB[`${member.guild.id}_${executor.id}_BANNED-MEMBER`];
        let limit = db.get(`BANS-LIMIT_${member.guild.id}`);

        if (!banAdd)
          tempDB[`${member.guild.id}_${executor.id}_BANNED-MEMBER`] = 0;
        if (!limit) db.set(`BANS-LIMIT_${member.guild.id}`, 1);
        if (banAdd >= limit - 1) {
          clearPermissions(member, client);
          ...
          tempDB[`${member.guild.id}_${executor.id}_BANNED-MEMBER`]++;
          setTimeout(() => {
            tempDB[`${member.guild.id}_${executor.id}_BANNED-MEMBER`] = 0;
          }, 20 * 1000);
